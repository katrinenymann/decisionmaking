...
# A good match
data=subset(np, UniqueID == "Original 2")
data= subset(data, Punishment.Round=="N")

slope = df$pbeta[2]


plot1 = ggplot(data, aes(x=Lagged.MCO, y=Contribution)) + 
  ...
  ylab("My contribution") +
  labs(color = "Condition") +
  ggtitle("A little off")

# Undetected free-rider 
data2=subset(np, UniqueID == "Original 3")
data2= subset(data2, Punishment.Round=="N")

slope = df$pbeta[3]

plot2 = ggplot(data2, aes(x=Lagged.MCO, y=Contribution)) + 
  geom_segment(mydat, mapping = aes(x = 0, xend = 20, y = 0, yend = mean(slope*20))) +
  geom_point() +
  xlim(0, 20) +
  ylim(0, 20) +
  ...
  ylab("My contribution") +
  labs(color = "Condition") +
  ggtitle("Undetected free-rider")

#
data=subset(np, UniqueID == "Original 10")
data= subset(data, Punishment.Round=="N")

slope = df$pbeta[10]
plot3 = ggplot(data, aes(x=Lagged.MCO, y=Contribution)) + 
  geom_segment(mydat, mapping = aes(x = 0, xend = 20, y = 0, yend = mean(slope*20))) +
  geom_point() +
  xlim(0, 20) +
  ylim(0, 20) +
  ...
  ylab("My contribution") +
  labs(color = "Condition") +
  ggtitle("Influenced by one outlier")

#
data=subset(np, UniqueID == "Original 120")
data= subset(data, Punishment.Round=="N")

slope = df$pbeta[100]
plot4 = ggplot(data, aes(x=Lagged.MCO, y=Contribution)) + 
  geom_segment(mydat, mapping = aes(x = 0, xend = 20, y = 0, yend = mean(slope*20))) +
  geom_point() +
  xlim(0, 20) +
  ylim(0, 20) +
  ...
  ylab("My contribution") +
  labs(color = "Condition") +
  ggtitle("A good match")


(plot1 + plot4) / (plot2 + plot3) +  plot_annotation(tag_levels = 'A')


data=subset(np, UniqueID == "Original 18")
data= subset(data, Punishment.Round=="N")
...
plot1 = ggplot(dat1, mapping = aes(x = xs, y = ys1, color = "Breakfast")) + 
  ...
  xlim(0, 20) +
  ylim(0, 20) +
  xlab("Lagged contribution of group / Belief about other' contribution") +
  ylab("My contribution") +
  labs(color = "Condition") +
  ggtitle("Mean pbeta slope for each condition") + 
  #geom_errorbar(aes(ymin = ys-sd, ymax = ys+sd)) +
  geom_ribbon(aes(y = ys1, ymin = ys1 - sd1, ymax = ys1 + sd1), alpha = .2) 

plot1 +  
  geom_line(dat0, mapping = aes(x = xs, y = ys0, color = "No breakfast")) + 
  geom_ribbon(aes(y = ys0, ymin = ys0 - sd1, ymax = ys0 + sd0), alpha = .2) +
  scale_color_manual(
      values = c(
           "No breakfast"="red",
           "Breakfast"="blue"))
...
# it seems like some plots work and other have bad valued maybe related to not enough trials. They never gained the right estimates. 
